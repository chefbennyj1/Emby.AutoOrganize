...
function (globalize, serverNotifications, events, datetime, loading, mainTabsManager, dialogHelper, taskButton) {
    ...
    async function openComparisonDialog2(id) {
        //THIS was being worked ok - using chef code atm
        //will need to reassess this
        var sourceResult = await ApiClient.getFileOrganizationResults(query)
        var item = sourceResult.Items.filter(r => r.Id == id)[0];
        window.y = item;
        var libraryResult = await ApiClient.getJSON( await ApiClient.getUrl('Items?Recursive=true&Fields=MediaStreams&IncludeItemTypes=' + item.Type + '&Ids=' + item.ExistingInternalId))
        ...
        dlg.querySelectorAll('.btnProcessResult').forEach(function(btn) {
            btn.addEventListener('click',
                async function (e) {
                    //let id = e.target.closest('button').getAttribute('data-resultid');
                    organizeFile(e.view, id);
        ...
        dlg.querySelectorAll('.btnDeleteResult').forEach(function(btn) {
            btn.addEventListener('click',
                async function (e) {
                    //let id = e.target.closest('button').getAttribute('data-resultid');
                    deleteOriginalFile(e.view, id);
    ...
    async function reloadItems(page, showSpinner, searchTerm = "") {
        ...
        page.querySelectorAll('.btnShowSubtitleList').forEach(btn => btn.addEventListener('click',
            (e) => {
                ...
                if (result) {
                    ...
                    var msg = "";
                    subtitles.forEach(t => { msg += t + '\n'; });

                    Dashboard.alert({ title: "Subtitles", message: msg });
                }
            }));
        ...
        var itemsToCompare = statusIcons.filter(icon => icon.dataset.status === "SkippedExisting" ||
            icon.dataset.status === "NewEdition" || icon.dataset.status === "NewResolution");
    ...
    function renderResults(page, result, paginate=true) {
        ...
        if (Object.prototype.toString.call(page) !== "[object Window]") {
            ...
            if (document.body.clientWidth > 12) {
                //just using an insanely small size to force during testing
            ...
            if (paginate == false) {
                //don't create pages if filtering
    ...
    function renderActionButtons(item) {
        ...
        if (item.Status === "Checking" || item.Status === "Processing" || item.Status === "InUse") return html;
        ...
        if (item.Status !== "Success") {
            ...
            if (item.Status !== "Processing") {
                ...
                if (item.TargetPath && (item.Status === "NewResolution" || item.Status === "SkippedExisting" || item.Status === "NewMedia" || item.Status === "NewEdition")) {
    ...
    async function onServerEvent(e, apiClient, data) {
        if (e.type === "TaskComplete") {
            if (data && data == 'AutoOrganize') {
                ...
                //keep progress always there but invisible for UI purposes
    ...
    async function updateItemStatus(page, item) {
        //need to update for grid layout
        ...
        processingAnimation = setInterval(() => {
            view.querySelectorAll('.statusIcon').forEach(svg => {
                if (svg.dataset.active === "true") {
                    ...
                    path.setAttribute("d", processSvgAnimationFrame(step))
                    step += 1;
